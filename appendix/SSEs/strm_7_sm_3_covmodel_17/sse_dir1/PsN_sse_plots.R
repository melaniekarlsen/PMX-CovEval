...
#Created 2025-05-15 at 17:40

xpose.runno <- '7_sm_3_covmodel_17.mod'
toolname <- 'sse'
pdf.filename <- paste0('PsN_',toolname,'_plots.pdf')
working.directory<-'/workmtp/content/trialmp/Melanie/data_simulation/results/SSEs/strm_7_sm_3_covmodel_17/sse_dir1/'
results.directory <- '..'
...
raw.results.file <- 'raw_results_strm_7_sm_3_covmodel_17.csv'
theta.labels <- c('POP_V2','POP_V3','POP_Q','POP_KA','POP_CL','POP_CLCLCR')
...
samples   <-200
model.file.names <-  c('strm_7_sm_3_covmodel_17.mod')
model.rawres.names <- c('simulation')
model.subjects <- c(639)
model.estimated.params <- c(11)
